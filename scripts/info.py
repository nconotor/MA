...
def plot_timeseries(df, sm='max'):
    ...
    plt.title(f'{plot_name} CPU Latencies over Time')
    plt.xlabel('Tick')
    plt.ylabel(f'{plot_name} Latency')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    for cpu, group in df.groupby('CPU'):
        ...
        plt.plot(group['Tick'], latency_values, linestyle='-', label=f'CPU {cpu}', alpha=0.7,
                 color=custom_colors[cpu - 1])
        plt.title(f'CPU {cpu} {plot_name} Latency over Time')
        plt.xlabel('Tick')
        plt.ylabel(f'{plot_name} Latency')
        plt.legend()
        plt.grid(True)
...
def plot_histogram(file_path, step=50, maxrange=400):
    ...
    for thread_id, group in grouped:
        ...
        ax_singles.bar(group['Latency'], group['Count'], width=1, log=True, color=custom_colors[int(thread_id) - 1],
                       linewidth=1)
        ax_singles.set_title(f'CPU {thread_id} Latency Histogram')
        ax_singles.set_xlabel('Latency (us)')
        ax_singles.set_ylabel('Number of latency samples')
    ...
    ax_merged.bar(grouped_by_latency.index, grouped_by_latency, width=1, log=True, color=custom_colors[0],
                  linewidth=1)
    ax_merged.set_title('Latency Histogram (All CPUs summed up)')
    ax_merged.set_xlabel('Latency (us)')
    ax_merged.set_ylabel('Number of latency samples')
